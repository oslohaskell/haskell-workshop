...
greaterThan2 list = filter (> 2) list
-- Alternatively:
--greaterThan2 = filter (> 2)
--greaterThan2 list = filter (\x -> x > 2) list
...
-- Alternatively:
--filterNot condition = filter (not . condition)
--filterNot condition list = filter (\x -> not (condition x)) list
